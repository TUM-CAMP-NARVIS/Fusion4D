...
#define CUBEPRUNE_NDIMS 3
	...
	__global__ void cubes_prune_with_depth_kernel(OccupcyCube *dev_cubes,
	...
	template <int dsStep>
	__global__ void cubes_prune_with_vts_kernel(OccupcyCube *dev_cubes,
	...
	__global__ void cubes_prune_with_static_volume_kernel(OccupcyCube *__restrict__ dev_cubes, float3 const *__restrict__ dev_ptr_cubes_offset, int3 const *__restrict__ dev_ptr_cubes_num, float cube_res,
	...
	__global__ void extract_occupied_cubes_and_set_cubes_offset_kernel(OccupcyCube *__restrict__ dev_cubes,
	...
	__global__ void setup_volume_dim_kernel(BoundingBox3DCuda const *__restrict__ dev_bbox,
	...
	__global__ void setup_volume_dim_kernel2(BoundingBox3DCuda const *__restrict__ dev_bbox_cur,
											 ...
											 float bbox_extension)
	{
		...
		else
		{
			...
			sh_corners[0][0] = bbox.x_s;
			sh_corners[1][0] = bbox.x_s;
			sh_corners[2][0] = bbox.x_s;
			sh_corners[3][0] = bbox.x_s;
			sh_corners[4][0] = bbox.x_e;
			sh_corners[5][0] = bbox.x_e;
			sh_corners[6][0] = bbox.x_e;
			sh_corners[7][0] = bbox.x_e;
			sh_corners[0][1] = bbox.y_s;
			sh_corners[1][1] = bbox.y_s;
			sh_corners[2][1] = bbox.y_e;
			sh_corners[3][1] = bbox.y_e;
			sh_corners[4][1] = bbox.y_s;
			sh_corners[5][1] = bbox.y_s;
			sh_corners[6][1] = bbox.y_e;
			sh_corners[7][1] = bbox.y_e;
			sh_corners[0][2] = bbox.z_s;
			sh_corners[1][2] = bbox.z_e;
			sh_corners[2][2] = bbox.z_s;
			sh_corners[3][2] = bbox.z_e;
			sh_corners[4][2] = bbox.z_s;
			sh_corners[5][2] = bbox.z_e;
			sh_corners[6][2] = bbox.z_s;
			sh_corners[7][2] = bbox.z_e;
	...
	__global__ void copy_volume_data_gpu_kernel(OccupcyCube *__restrict__ dev_cubesDst,
												int3 const *__restrict__ dev_ptr_cubes_num,
	...
	// one thread one block
	__global__ void extract_volume_bbox_kernel(BoundingBox3DCuda *dev_bbox,
...
#pragma unroll
					for (int i = 0; i < dev_num_cam_views; i++)
					{
						...
						if (X[2] > 0.1f)
						{
							...
							if (u >= 0 && u < depth_width &&
								v >= 0 && v < depth_height)
							{
								unsigned short ds = tex2DLayered<unsigned short>(tex_depthImgs, u, v, i);
								if (bUseVisualHull)
									depth_extract_fg_set_bg_as_far(ds);
								else
									depth_extract_fg(ds);

								float4 nd_ = tex2DLayered<float4>(tex_normalMaps, u, v, i);
	...
	__global__
		__launch_bounds__(512, 3) void update_volume_with_curr_depth_f2f_kernel_vResidual(VolumeDataGPU volume,
																						  float cube_res, int cube_size_in_vxl, float vxl_res, int vxls_per_cube,
...
#pragma unroll
	...
	template <class T>
	__global__ void init_two_level_hierachy_volume_data_kernel(T *__restrict__ dev_vxl_data,
	...
	__global__ void warp_volume_f2f_kernel_ngns(int *cube_ngns_indices,
	...
	__global__
		__launch_bounds__(512, 2) void warp_and_update_volume_f2f_kernel(VolumeConstDataGPU volume_src, VolumeDataGPU volume_dst,
...
#pragma unroll
		for (int i = 0; i < 4; i++)
			expval[i] = __expf(-(i) / (2.0f * sigma_s * sigma_s));
						...
						for (int k = -1; k <= 1; k++)
						{
							...
							for (int j = -1; j <= 1; j++)
							{
									...
									0 <= zId_n && zId_n < 10)
								{
									...
									float val[4];
									if (xId > 0)
										val[0] = sh_volume[idx_sh_n - 1];
									else
										val[0] = -2;
									val[1] = sh_volume[idx_sh_n];
									if (xId < 9)
										val[2] = sh_volume[idx_sh_n + 1];
									else
										val[0] = -2;
									if (xId < 8)
										val[3] = sh_volume[idx_sh_n + 2];
									else
										val[0] = -2;
...
#pragma unroll
	...
	__global__ void correct_volume_weights_kernel(float *__restrict__ vxl_data, float *__restrict__ vxl_weights, uchar4 *__restrict__ vxl_colors,
												  float *__restrict__ dev_weights_interp, int *__restrict__ dev_counts_interp,
	...
	__global__
		__launch_bounds__(512, 2) void warp_volume_Step1_save_vxl_pos_kernel(ushort3 *__restrict__ dev_vxl_pos_t,
...
#pragma unroll
	...
	__global__ void warp_volume_Step2_vote_min_sdf_at_dst_kernel(float *__restrict__ dev_vxl_min_sdf_dst,
...
#pragma unroll
	...
	__global__ void warp_volume_Step3_get_min_sdf_srcIdx_kernel(int *__restrict__ dev_vxl_min_sdf_srcIdx_dst,
...
#pragma unroll
	...
	__global__
		__launch_bounds__(512, 2) void warp_volume_Step4_cast_sdf(ushort3 const *__restrict__ dev_vxl_pos_t,
