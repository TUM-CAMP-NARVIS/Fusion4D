#ifndef __EDMATCHINGHELPERCUDAIMPL_CUH__
#define __EDMATCHINGHELPERCUDAIMPL_CUH__
...
#define JTJ_BLKS_NUM_MAX            \
	(ED_NODES_NUM_MAX * EDNODE_NN + \
	 ED_NODES_NUM_MAX * JTJ_BLKS_NUM_INDUCED_PER_VERTEX)
...
extern __constant__ __device__ CameraViewCuda dev_cam_views[MAX_NUM_DEPTH_CAMERAS];
extern __constant__ __device__ int dev_num_cam_views;
extern int host_num_cam_views;
...
public:
	...
	void clear_jtj_jtf()
	{
		checkCudaErrors(cudaMemset(
			this->dev_jtj_, 0, sizeof(float) * this->jtj_blks_num_.max_size * 144));
		checkCudaErrors(
			cudaMemset(this->dev_jtf_, 0,
					   sizeof(float) * this->jtj_para_blks_num_.max_size * 12));
	}
	...
	bool compute_jtj_jtf(float *dev_vts, float *dev_vts_t,
						 cuda::gpu_size_data vts_num_gpu, int vt_dim,
						 DeformGraphNodeCuda const *dev_ed_nodes,
						 RigidTransformCuda const *dev_rigid_transf,
						 double w_data, double w_vis_hull, double mu,
						 bool bEveluateJtJOffDiag = true);

	bool compute_jtj_jtf_reg(DeformGraphNodeCuda *dev_ed_nodes,
							 DeformGraphNodeCoreCuda *dev_ed_nodes_initial,
							 int const *dev_ed_nodes_num_all_levels,
							 float min_nodes_dist, float w_reg);
	bool compute_jtj_jtf_reg_vRobust(
		DeformGraphNodeCuda *dev_ed_nodes,
		DeformGraphNodeCoreCuda *dev_ed_nodes_initial,
		int const *dev_ed_nodes_num_all_levels, float min_nodes_dist, float tau,
		float w_reg);
	bool compute_jtj_jtf_rot(DeformGraphNodeCuda *dev_ed_nodes,
							 int const *dev_ed_nodes_num_all_levels,
							 double w_reg);
	void compute_jtj_jtf_temporal(
		DeformGraphNodeCuda const *dev_ed_nodes,
		DeformGraphNodeCoreCuda const *dev_ed_nodes_initial,
		RigidTransformCuda const *dev_rigid_transf,
		RigidTransformCuda const *dev_rigid_transf_prev,
		int const *dev_ed_nodes_num_all_levels, float w_temporal);
	void compute_jtj_jtf_temporal_vRobust(
		DeformGraphNodeCuda const *dev_ed_nodes,
		DeformGraphNodeCoreCuda const *dev_ed_nodes_initial,
		RigidTransformCuda const *dev_rigid_transf,
		RigidTransformCuda const *dev_rigid_transf_prev,
		int const *dev_ed_nodes_num_all_levels, float tau_A, float tau_t,
		float w_temporal);

	void set_gpu_costs_to_zero();
	void sum_over_gpu_costs(float *dev_cost_sum);
	void evaluate_cost(float const *dev_vts, float const *dev_vts_t,
					   cuda::gpu_size_data vts_num_gpu, int vt_dim, double w_data,
					   double w_vis_hull, double mu, float2 *costs = NULL);
	void evaluate_cost_reg(DeformGraphNodeCuda *dev_ed_nodes,
						   DeformGraphNodeCoreCuda *dev_ed_nodes_initial,
						   int const *dev_ed_nodes_num_all_levels,
						   float min_nodes_dist, float w_reg, float *cost = NULL);
	void evaluate_cost_reg_vRobust(DeformGraphNodeCuda *dev_ed_nodes,
								   DeformGraphNodeCoreCuda *dev_ed_nodes_initial,
								   int const *dev_ed_nodes_num_all_levels,
								   float min_nodes_dist, float tau, float w_reg,
								   float *cost = NULL);
	void evaluate_cost_rot(DeformGraphNodeCuda *dev_ed_nodes,
						   int const *dev_ed_nodes_num_all_levels, double w_rot,
						   float *cost = NULL);
	void evaluate_cost_temporal(
		DeformGraphNodeCuda const *dev_ed_nodes,
		DeformGraphNodeCoreCuda const *dev_ed_nodes_initial,
		RigidTransformCuda const *dev_rigid_transf,
		RigidTransformCuda const *dev_rigid_transf_prev,
		int const *dev_ed_nodes_num_all_levels, float w_temporal,
		float *cost = NULL);
	void evaluate_cost_temporal_vRobust(
		DeformGraphNodeCuda const *dev_ed_nodes,
		DeformGraphNodeCoreCuda const *dev_ed_nodes_initial,
		RigidTransformCuda const *dev_rigid_transf,
		RigidTransformCuda const *dev_rigid_transf_prev,
		int const *dev_ed_nodes_num_all_levels, float tau_A, float tau_t,
		float w_temporal, float *cost = NULL);

	bool setup_vt_data_for_jtj(
		DeformGraphNodeCuda const *dev_ed_nodes,
		cuda::gpu_size_data ed_nodes_num_gpu, int *dev_ngns_indices,
		float *dev_ngns_weights, int vts_num,
		int thres_pts_num = 100 // ignore jtj off-diagnal block if its associates
								// points is leff than thres_pts_num
	);

	bool setup_ed_nodes_reg_term(short2 *dev_jtj_2d_infos,
								 DeformGraphNodeCuda *dev_ed_nodes,
								 int ed_nodes_num, cudaArray *cu_3dArr_ndIds,
								 int3 ed_cubes_dims, float3 ed_cubes_offsets,
								 float ed_cube_res, float nodes_min_dist);

	void setup_vt_data_for_jtj_vHierED(
		DeformGraphNodeCuda const *dev_ed_nodes,
		cuda::gpu_size_data ed_nodes_num_gpu,
		int const *dev_ed_nodes_num_all_levels, int *dev_ngns_indices,
		float *dev_ngns_weights, cuda::gpu_size_data vts_num_gpu,
		int thres_pts_num = 100 // ignore jtj off-diagnal block if its associates
								// points is leff than thres_pts_num
	);

	void setup_ed_nodes_reg_term_vHierED(
		DeformGraphNodeCuda *dev_ed_nodes, int const *dev_ed_nodes_num_all_levels,
		int2 *dev_ed_nodes_ranges[], int ed_hierachy_levels,
		int3 const *ed_cubes_dims, float3 const *ed_cubes_offsets,
		float ed_cube_res,
...
public:
	void allocate_memory_visibility_check(
		...
		int down_scale_trg);
	void project_points_to_depth_map(float const *dev_vts_t,
									 cuda::gpu_size_data vts_num_gpu, int vt_dim,
									 float vxl_res, bool bSaveFrontVtIdx = false);
	void calc_alignment_residual_on_depth(int filter_radius, float residual_cap);
	void calc_alignment_residual_on_depth(VolumeTwoLevelHierachy const *volume,
										  int filter_radius);
	...
	float const *dev_depth_align_residual_f()
	{
		return dev_depth_align_residual_f_;
	}
...
public:
	void allocate_memory_ed_vertex_residual(int vts_num_max,
											int ed_nodes_num_max);
	void calc_per_vertex_residual(float const *dev_vts_t,
								  cuda::gpu_size_data vts_num_gpu, int vt_dim,
								  float residual_cap, bool bUseSegmentation);
	void calc_per_vertex_residual(float const *dev_vts_t,
								  cuda::gpu_size_data vts_num_gpu, int vt_dim,
								  VolumeTwoLevelHierachy const *volume,
								  bool bUseVisualHull = false);

	void aggregate_ed_nodes_residual(cuda::gpu_size_data ed_nodes_num_gpu);
	void update_ed_nodes_residual_with_reg_term(
		DeformGraphNodeCuda const *dev_ed_nodes,
		cuda::gpu_size_data ed_nodes_num_gpu, float thres_reg,
		float delta_on_ed_residual);
	void interp_per_vertex_residual_from_ed(int const *dev_ngns_indices,
											float const *dev_ngns_weights,
											int vts_num, int ed_nodes_num);
	float const *dev_per_vertex_align_residual()
	{
		return this->dev_per_vertex_align_residual_;
	}
	float const *dev_per_vertex_align_residual_ed_interp()
	{
		return this->dev_per_vertex_align_residual_ed_interp_;
	}
	float const *dev_ed_nodes_align_residual()
	{
		return this->dev_ed_nodes_align_residual_;
	}
...
public:
	...
	void run_rigid_alignment(float const *dev_vts,
							 cuda::gpu_size_data vts_num_gpu, int vt_dim,
							 float mu, int iter_max);

	// rigid_transf.R = R * rigid_transf.R
	// rigid_transf.T = R*rigid_transf.T + T
	void apply_rigid_alignment_to_RigidTransf(
		RigidTransformCuda *dev_rigid_transf);

	void init_rigid_alignment_to_identity();
	void set_rigid_alignment(float rod[3], float T[3]);
	void compute_jtj_jtf_rigid_alignment(float const *dev_vts,
										 cuda::gpu_size_data vts_num_gpu,
										 int vt_dim, float mu);
	void evaluate_cost_rigid_alignment(float const *dev_vts, int vts_num,
									   int vt_dim, float mu);
	void solve_and_update_rod_T_rigid_alignment();
	void solve_rigid_alignment(float opt_mu);
	// debug
	void readout_R_T_rigid_alignment(float rod[3], float R[9], float dR_drod[27],
									 float T[3]);
	...
	int partial_jtjs_rigid_alignment_count()
	{
		return this->partial_JtJs_rigid_align_count_;
	}
...
public:
	...
	void calc_3d_keypts_in_reference(float3 *dev_keypts_3d,
									 ushort3 const *dev_keypts_2d,
									 float const *dev_vts, float const *dev_vts_t,
									 int vt_dim, int const *dev_depth_maps_proj,
									 int const *dev_depth_maps_vtIdx,
									 int depth_width_prj, int depth_height_prj);
	void calc_3d_keypts_in_curr(float3 *dev_keypts_3d,
								ushort3 const *dev_keypts_2d);
	void setup_keypts(float sigma_vt_node_dist);
	void compute_ngns_keypts(float sigma_vt_node_dist);
	void evaluate_cost_keypts(DeformGraphNodeCuda const *dev_ed_nodes,
							  RigidTransformCuda const *dev_rigid_transf,
							  float w_keypts, float *cost = NULL);
	void evaluate_cost_keypts_vRobust(DeformGraphNodeCuda const *dev_ed_nodes,
									  RigidTransformCuda const *dev_rigid_transf,
									  float w_keypts, float tau,
									  float *cost = NULL);
	void compute_jtj_jtf_keypts(DeformGraphNodeCuda const *dev_ed_nodes,
								int const *dev_ed_nodes_num,
								RigidTransformCuda const *dev_rigid_transf,
								float w_keypts, bool bEveluateJtJOffDiag = true);
	void compute_jtj_jtf_keypts_vRobust(
		DeformGraphNodeCuda const *dev_ed_nodes, int const *dev_ed_nodes_num,
		RigidTransformCuda const *dev_rigid_transf, float w_keypts, float tau,
		bool bEveluateJtJOffDiag = true);
...
public:
	...
	bool allocate_and_texBind_cuda_arrays_depth(int tex_num, int width,
												int height);
...
public:
	...
	cuda::gpu_size_data const &jtj_para_blk_num() const
	{
		return this->jtj_para_blks_num_;
	}
	float *dev_mem_jtj() { return this->dev_jtj_; }
	float *dev_mem_jtf() { return this->dev_jtf_; }
	HessianBlockInfoCuda *dev_mem_blk_info()
	{
		return this->dev_jtj_blk_info_buf_;
	}

public:
	void allocate_and_bind_vishull_cu3dArray();
	// setup visual hull
	void setup_visual_hull_info(BoundingBox3DCuda const *dev_bbox,
								float visual_hull_res);
...
protected:
	...
	// 2d matrix (nodes_num X nodes_num). each element saves the data offset (in
	// x) and flag(in y, lowest bit data term flag, second lowest bit reg term
	// flag)
...
__forceinline__ __device__ void robust_kernel_paras(float r, float tau,
													float &c, float &d,
													float &e)
{
	float a, b;
...
// Kernel declaration for cross-translation-unit kernel
__global__ void init_short2_array_2d(short2 *p_array, int dim, short2 val);

#endif // __EDMATCHINGHELPERCUDAIMPL_CUH__
