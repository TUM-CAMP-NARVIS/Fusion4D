...
namespace VolumetricFusionCuda
{
	...
	__global__ void marching_cubes_kernel_vMesh_stepVertex(VolumeConstDataGPU volume,
														   ...
														   float iso_level = 0.0f)
	{
		...
		for (int blkId = blockIdx.x; blkId < count_occupied_cube; blkId += gridDim.x)
		{
			...
			{
				int tableIdx = 0;
				if (-1.0f < sh_volume[idx_sh] && sh_volume[idx_sh] < iso_level)
					tableIdx |= 1;
				if (-1.0f < sh_volume[idx_sh + (resoFactor * 9)] && sh_volume[idx_sh + (resoFactor * 9)] < iso_level)
					tableIdx |= 2;
				if (-1.0f < sh_volume[idx_sh + (resoFactor * 9) + (resoFactor * 1)] && sh_volume[idx_sh + (resoFactor * 9) + (resoFactor * 1)] < iso_level)
					tableIdx |= 4;
				if (-1.0f < sh_volume[idx_sh + (resoFactor * 1)] && sh_volume[idx_sh + (resoFactor * 1)] < iso_level)
					tableIdx |= 8;
				if (-1.0f < sh_volume[idx_sh + (resoFactor * 81)] && sh_volume[idx_sh + (resoFactor * 81)] < iso_level)
					tableIdx |= 16;
				if (-1.0f < sh_volume[idx_sh + (resoFactor * 81) + (resoFactor * 9)] && sh_volume[idx_sh + (resoFactor * 81) + (resoFactor * 9)] < iso_level)
					tableIdx |= 32;
				if (-1.0f < sh_volume[idx_sh + (resoFactor * 81) + (resoFactor * 9) + (resoFactor * 1)] && sh_volume[idx_sh + (resoFactor * 81) + (resoFactor * 9) + (resoFactor * 1)] < iso_level)
					tableIdx |= 64;
				if (-1.0f < sh_volume[idx_sh + (resoFactor * 81) + (resoFactor * 1)] && sh_volume[idx_sh + (resoFactor * 81) + (resoFactor * 1)] < iso_level)
					tableIdx |= 128;
	...
	__global__ void marching_cubes_kernel_vMesh_stepTriangle(VolumeConstDataGPU volume,
	...
	template <class T>
	__global__ void init_buf(T *dev_buf, T val, int buf_size)
	{
